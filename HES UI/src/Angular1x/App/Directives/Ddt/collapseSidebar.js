'use strict'
app.directive('collapseSidebar', function (s) {
    return {
        restrict: 'A',
        link: function postLink(scope, element) {
            ...
            element.on('click', function (e) {
                var brandTitleEle = document.getElementById('brandTitleId');
                if (app.hasClass('sidebar-sm')) {
                    app.removeClass('sidebar-sm').addClass('sidebar-xs');
                    
                    brandTitleEle.style.visibility = 'hidden';
                }
                else if (app.hasClass('sidebar-xs')) {
                    app.removeClass('sidebar-xs');
                    brandTitleEle.style.visibility = 'visible';
                }
                else {
                    app.addClass('sidebar-sm');
                    brandTitleEle.style.visibility = 'visible';
                }
                var elementResult = element[0].getElementsByClassName('PGCIL_Name');
